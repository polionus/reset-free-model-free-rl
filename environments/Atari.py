...
from src.glue.environment import BaseEnvironment
from utils.distance_rep import get_DQN_atari_distance_rep
...
class Atari(BaseEnvironment):
    def __init__(self, game: str, seed: int, repeat_action_probability: float, frameskip: int, max_steps: Optional[int] = None, distance_rep_path: Optional[str] = None):
        self.env = gymnasium.make(f'ALE/{game}-v5', max_episode_steps=max_steps, frameskip=1, repeat_action_probability=0.0)
        self.seed = seed
        self.max_steps = max_steps
        self.rng = np.random.default_rng(self.seed)
        self.noop_max = 30 if repeat_action_probability == 0.0 else 0
        ...
        self._last_lives = 0
        self._frame_skip = frameskip
        self._frame_buffer = deque(maxlen=2)

        # distance rep
        if distance_rep_path is not None:
            self.distance_rep = get_DQN_atari_distance_rep(distance_rep_path)
        else:
            self.distance_rep = lambda _: None
    ...
    def start(self):
        ...
        noops = (
            self.rng.integers(1, self.noop_max + 1)
            if self.noop_max > 0
            else 0
        )
        for _ in range(noops):
            s, _, t, truncated, step_info = self.env.step(NOOP_ACTION)
        ...
        stacked_obs = self._stacker.next(obs)
        extra = {
            'distance_rep': self.distance_rep(stacked_obs)
        }
        return stacked_obs, extra
    ...
    def step(self, action):
        ...
        for frame in range(self._frame_skip):
            ...
            elif self.rng.random() > self.repeat_action_probability:
                self.last_action = action

            sp, r, t, _, info = self.env.step(self.last_action)
        ...
        extra = {
            'gamma': gamma,
            'distance_rep': self.distance_rep(obs)
        }

        return (total_reward, obs, terminated, extra)
